...
export default function ManagerPayoutSessionRow({
  sessionId,
  sessionTitle,
  scheduledAt,
  transactionCount,
  ...
  const statusLabel = getStatusLabel(status, payoutId);
  const scheduledAtLabel = formatDateTime(scheduledAt);
  ...
  return (
    <div className="flex flex-col gap-3 py-4 lg:grid lg:grid-cols-[minmax(0,2fr)_140px_200px_auto] lg:items-center lg:gap-4">
      <div className="min-w-0 space-y-1">
        <p className="truncate text-base font-semibold">{sessionTitle}</p>
        <p className="text-sm opacity-60">
          {sessionId ? `Session #${sessionId}` : "Sessionless entries"}
        </p>
        {scheduledAtLabel && (
          <p className="text-xs opacity-60">Scheduled {scheduledAtLabel}</p>
        )}
        {(statusLabel || payoutId || paidAtLabel) && (
